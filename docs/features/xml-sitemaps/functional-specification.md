...
* `{{n}}` is the pagination state (ignoring the first page in the series) (e.g., `post-sitemap.xml`, `post-sitemap2.xml`, `custom_taxonomy_name-sitemap.xml`).
...
* **(Optional)** `publication_date` is the timestamp at which the video was 'published' (alternatively, *uploaded*)
